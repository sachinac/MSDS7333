...
df <- read.csv("cherry_blossom_final.csv",na.strings = c("NR", "NA"))
...
df$ageRange = cut(df$age,breaks = c(15,25,45,65,90))

age_yr_cnt <- na.omit(df %>% group_by(ageRange) %>% count(year) %>% mutate(Nor = n/n[1])) %>% mutate(n_diff = n - lag(n))

df$ageRange_gran = cut(df$age,breaks = c(seq(15,75,10),90))

age_gran_yr_cnt <- na.omit(df %>% group_by(ageRange_gran) %>% count(year) %>% mutate(Nor = n/n[1])) %>% mutate(n_diff = n - lag(n))
...
ggplot(age_yr_cnt, aes(x = ageRange, y = n_diff, fill = as.factor(year)))+
  ...
  labs(title="Women Participation Increment \n per Year by Age Group",x="Year", y = "#Women")+
  scale_fill_discrete()+
  theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(age_yr_cnt, aes(x = year, y = n))+
  ...
  labs(title="Women Participation by Year",x="Year", y = "#Women")+
  theme_minimal()+
  theme(plot.title = element_text(hjust = 0.5))
```

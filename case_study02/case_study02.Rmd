...
ggplot(df, aes(x=year, y=total_womens)) + 
  ...
  labs(title="5a) Women Participation", 
       subtitle="1999-2012", 
       x="Year of Race",
...
ggplot(age_gran_yr_cnt, aes(x = year, y = n))+
  ...
  labs(title="5b) Women Participation \n by Age Group",x="Year of Race", y = "Womens Participants")+
  theme_minimal()+
...
Looking into the participation, its going up over time, in 1999 they had around 2,500 participants and it went up to 100,000 in 2012 almost a 4x increase in participation.
...
ggplot(cb_final_ds, aes(x = ageRange_gran,y = mins,fill=as.factor(ageRange_gran)))+
   geom_boxplot() + 
   labs(title="6a) Runtime Distribution by Age Group", 
        subtitle="1999-2012",
         caption="Source: Cherry Blossom",
         x="Age Group",
...
ggplot(cb_final_ds, aes(x = factor(year), y = mins,fill=as.factor(year)))+
   geom_boxplot()+
   labs(title="6b) Runtime Distribution by Year",
        subtitle="1999-2012",x="Year", y = "Runtime, minutes") + theme(legend.position="none")
...
### Longitudinal Analysis of Individual runner 
...
Shows individual performances of women runners who have completed at least 13 races.There are total 9 runners found in the dataset.We can clearly see trend and performance of these 9 players over the years. As it can be seen in this plot most of the players started their competition in early forty years of age and performance of most of them are not consistent however, combined plot looks going upward which means that performances affected as age increases. i.e. time to finish race increases with age. There is only player seen started her participation in early twenties but her performance went down in late thirties. 

8b) 

Shows individual performances of women runners who have completed at least 10 races. There are total 70 runners in this category. Although its quite difficult to look for invidual performances, we can see overall trend is showing upward i.e. impact of age on runner's time.
...
cb_final_ds %>% filter(year==1999 | year==2012) %>% 
            ...
            geom_density() + 
  labs(title="Age Distribution by Year",x="Age", y = "Density",fill='Year')

cap <- "Age Distribution change 1999 Vs 2012"
```
Clear shift to younger population in recent years is noted. Looking at kernel density plots between 1999 and 2012, it is clear that there is shift towards younger age group.
...
Break up of race times by year, and reviewing median, fastest and slowest runtime provides insight on over all race time trend through the years. The median runtime and fastest runtime do not show as much variation as the slowest Runtime. The standard deviation for different time metrics over the years is captured in the below Table.

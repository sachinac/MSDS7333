...
VIM::marginplot(cb_processed_ds[,c(4,12)],main= "2a) Margin Plot")
#
...
naniar::gg_miss_var(cb_raw_ds[-c(which(is.na(cb_raw_ds$age))),-c(1,3,12)],show_pct = TRUE)+labs(title= "2b) After handling Missing Values ")
...
```{r, echo=FALSE,out.width='.49\\linewidth', fig.width=7, fig.height=6,fig.show='hold',fig.align='left',fig.cap='Year wise Distribution'}
...
```
The Age Distribution chart from 1999-2012 shows over the course of time the average age has remained relatively constant. 25-30 year olds make up majority of the race population and that's consistent throughout the years the races were held.
...
ggplot(df, aes(x=year, y=total_womens)) + 
  ...
  labs(title="Women's Participation in Race", 
       subtitle="1999-2012", 
       x="Year of Race",
...
#cb_final_ds$age <- as.numeric(cb_final_ds$age)
#cb_final_ds$PaceMins <- as.character(cb_final_ds$PaceMins)

yr_cnt <- count(cb_final_ds,year)

tformat <- hms(cb_final_ds$time)
#cb_final_ds$tmins <- hour(tformat)*60 + minute(tformat)  + second(tformat)/60
...
#age_yr_time <- (cb_final_ds %>% group_by(ageRange_gran,year) %>% mutate(t_median = median(tmins)) %>% mutate(t_min = min(tmins)))
...
ggplot(na.omit(cb_final_ds), aes(x = ageRange_gran, y = mins))+
  geom_boxplot()+
...
ggplot(cb_final_ds, aes(x = factor(year), y = mins))+
  geom_boxplot()+

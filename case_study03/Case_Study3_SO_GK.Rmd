...
cp_val <- c(0.0001,seq(0.0001,0.1,0.001),0.1)
...
f       <- vector(length = length(cp_val))
prec    <- vector(length = length(cp_val))
rec     <- vector(length = length(cp_val))
...
for (i in 1:length(cp_val)) {
  ...
  model = rpart(formula=isSpam~.,
                ...
                control = list(minsplit=50,cp = cp_val[i],maxdepth=50)
                )
  predTest <- predict(object=model, newdata=test_df, type="class")
  cm    <- confusionMatrix(data=predTest, reference=test_df$isSpam, positive ="T",mode="prec_recall")
...
df_cp_wide <- data.frame(cp = cp_val, accuracy = acc, specificity = spec, sensitivity = sens,F_Score=f,precsn = prec,reca =rec)

keycol <- "metric"
valuecol <- "value"
gathercols <- c("accuracy", "sensitivity", "specificity","F_Score")

...
ggplot(data=gg)+
  ...
  scale_x_log10()+
  labs(title="F1 Score Vs. Complexity Factor",x="cp", y = "F1 Score")
```
...
# ggplot(data=gg)+
#   geom_point(aes(x = cp, y=ppv.test.mean, color = factor(maxdepth)))+
#   # scale_color_grey()+
#   theme_light()+
#   theme(plot.title = element_text(hjust = 0.5),panel.grid.minor = element_line(colour="grey", size=0.1),panel.grid.major = element_line(colour="grey", size=0.3))+
#   scale_x_log10()+
#   labs(title="Precision Vs. Complexity Factor",x="cp", y = "Precision")
# 
# ggplot(data=gg)+
#   geom_point(aes(x = ppv.test.mean, y=f1.test.mean, color = factor(maxdepth), alpha = factor(minbucket)))+
#   # scale_color_grey()+
#   theme_light()+
#   theme(plot.title = element_text(hjust = 0.5),panel.grid.minor = element_line(colour="grey", size=0.1),panel.grid.major = element_line(colour="grey", size=0.3),legend.text = element_text(size=4))+
#   scale_x_log10()+
#   labs(title="F1 score Vs. Precision",x="Precision", y = "F1 Score")
...
#rpart.plot(model2, roundint = FALSE, box.palette = "BuBn", type = 5)
#summary(model)
#predTest = predict(object=model2, newdata=test.data, type="class")
#confusionMatrix(table(predTest,test.data$isSpam)[c(2,1),c(2,1)],mode = "everything")
...
An Email SPAM filtering model is developed capable of accurately identifying SPAM 90% of time, and with even better true negative rate, or specificity at 95%. This implies very few times a non-SPAM email gets classified as SPAM, and also very few times SPAM gets classified as non-SPAM.

A decision tree based classification approach is developed that depends on automatically reading the input Email and doing a real time check on it, and classifying the email as a SPAM or not SPAM.
...
In following circumstances tied to email construction, it classifies email as SPAM:
* forwards < 6.1, and perCaps >= 13, and numLines >9.5
* forwards < 6.1, and perCaps < 13, and perHTML > 3.9, and numLines >242
* forwards < 6.1. and perCaps < 13, and perHTML < 3.9, and numDlr > 0.5, isInReplyTo is true, and numLines > 103
* forwards < 6.1. and perCaps < 13, and perHTML < 3.9, and numDlr > 0.5, isInReplyTo is true, and numLines > 103, and numDlr >=9.
